...
const dbUrl = process.env.DATABASE_URL.split('?')[0]; // remove ?schema=public
const dbPrisma = postgres(dbUrl);
...
async function main() {
  const invs = await dbPrisma`SELECT * FROM "Inventario" ORDER BY "criadoEm" DESC LIMIT 1`;
  const inv = invs[0];
  if (!inv) return;
  console.log('Inventario:', inv.id, 'Data:', inv.criadoEm);

  const itens = await dbPrisma`SELECT * FROM "ItemInventario" WHERE inventario_id = ${inv.id}`;
...
main().catch(console.error).finally(() => { dbPrisma.end(); dbReadonly.end(); });
