...
// ─── Tipo do item bipado (apenas dados locais para máxima velocidade) ────────
type ItemSimples = {
  id: string;
...
function ItemRow({ 
  ...
  return (
    ...
    >
      {/* Sequencial # */}
      <td className="px-4 py-3 whitespace-nowrap text-xs font-mono font-semibold text-slate-400">
        #{numero}
      </td>
...
export default function InventarioPage() {
  ...
  // Timer para sincronização de fundo debounced
  const invalidateDebounceTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  // Queries e Mutations do tRPC
  const trpcUtils = api.useUtils();
  const { data: itens, isLoading, isFetching, refetch } = api.inventory.listarItensDoInventario.useQuery(
    { inventarioId },
    { 
      enabled: !!inventarioId,
      // Auto-refresh a cada 4s para manter sempre a lista atualizada
      refetchInterval: 4000,
      refetchIntervalInBackground: false,
    }
  );

  // Busca o inventário para descobrir o status e o unidade_id → nome da unidade
  ...
  const isFinalizado = inventarioAtual?.status === "CONCLUIDO";
  ...
  const mutation = api.inventory.processarBipagemDiaria.useMutation({
    ...
    onSuccess: (data, variables, context) => {
      ...
      // Sincronização em background com debounce: só invalida se não houver mutações pendentes e após 2s
  ...
  // Re-foca o input sempre que a lista de itens for atualizada
  useEffect(() => {
    inputRef.current?.focus();
  }, [itens]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <header className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <Link
                href="/"
                className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-green-700 transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              ...
              {isFinalizado && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-200">
                  ...
                  Inventário Finalizado — Adição de Volumes
                </span>
              )}
            </div>
          ...
          <div className="flex items-center gap-3">
            <Link
              href={`/inventario/${inventarioId}/relatorio`}
              className="inline-flex items-center gap-2 px-4 py-2 bg-white text-slate-700 hover:text-slate-900 border border-slate-200 rounded-xl text-sm font-semibold shadow-sm hover:bg-slate-50 transition-colors"
            >
            ...
            <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-xl border border-slate-200 shadow-sm">
              ...
              <span className="text-xs font-semibold text-green-700">Leitor Ativo</span>
            </div>
          </div>
        </header>

        {/* ── Contadores resumo e Auto-refresh ── */}
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-white rounded-2xl border border-slate-200 px-4 py-3 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-3xl font-black text-slate-800">{todosItens.length}</p>
            ...
            >
              <svg className={`w-5 h-5 ${isFetching ? 'animate-spin text-indigo-600' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <form onSubmit={handleSubmit} className="relative group flex-1">
            ...
            <input
              ...
              placeholder="Aguardando código de barras (17 dígitos)..."
              className={[
          ...
          <button
            ...
            className="px-6 py-5 bg-white text-indigo-600 rounded-2xl font-semibold hover:bg-indigo-50 transition-colors border-2 border-indigo-100 flex items-center justify-center gap-2 whitespace-nowrap shadow-sm"
          >
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
            <h2 className="font-semibold text-slate-700">Volumes lidos neste inventário</h2>
          ...
          <div className="overflow-x-auto max-h-[60vh] overflow-y-auto">
            {isLoading ? (
              <div className="p-8 text-center text-slate-400">Carregando volumes...</div>
            ) : !todosItens.length ? (
              <div className="p-12 text-center text-slate-400 flex flex-col items-center">
                ...
                <p className="text-sm mt-1">Aponte o leitor de código de barras ou use a digitação manual acima.</p>
              </div>
            ) : (
                ...
                <tbody>
                  {todosItens.map((item, index) => (
                    <ItemRow
                      key={item.id}
                      item={item}
                      numero={todosItens.length - index}
        ...
        {/* ── Botão Finalizar / Ver Relatório ── */}
        <div className="flex justify-between items-center pb-8 flex-wrap gap-4">
          <Link
            href="/"
            className="inline-flex items-center gap-2 px-6 py-3.5 bg-white text-slate-600 hover:text-slate-800 border border-slate-200 rounded-2xl font-semibold shadow-sm hover:bg-slate-50 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {isFinalizado ? (
            <Link
              href={`/inventario/${inventarioId}/relatorio`}
              className="inline-flex items-center justify-center px-8 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl transition-all shadow-lg hover:shadow-xl group"
            >
      ...
      {modalManualAberto && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl transform transition-all">
            ...
            <p className="text-sm text-slate-500 mb-6">Insira o código de barras de 17 dígitos que não pôde ser lido pelo scanner.</p>
